<template>
  <div class="good-item">
    <div style="">
      <div class="good-img">
        <a :href="msg.ad_link">
          <img v-lazy="msg.ad_code" :alt="msg.productName">
        </a>
      </div>
      <h6 class="good-title" v-html="msg.ad_name">{{msg.ad_name}}</h6>
      <div class="good-price pr">
        <div class="ds pa">
          <a :href="msg.ad_link">
            <y-button text="查看详情" style="margin: 0 5px"></y-button>
          </a>
          <y-button text="加入购物车"
                    style="margin: 0 5px; display: none"
                    @btnClick="_addCart(msg.goods_id ,msg.productName, msg.goods_img)"
                    classStyle="main-btn"
          ></y-button>
        </div>
        <p><span style="font-size: 16px">￥</span>
          {{msg.goods_price}}</p>
      </div>
    </div>
  </div>
</template>
<script>
  import YButton from '/components/YButton'
  import { addCart } from '/api/cart.js'
  import { userInfo } from '/api/index.js'
  import { mapMutations, mapState } from 'vuex'
  export default {
    props: {
      msg: {}
    },
    data () {
      return {}
    },
    methods: {
      ...mapMutations(['UPDATE_USER', 'ADD_ANIMATION', 'SHOW_CART']),
      goodsDetails (id) {
        this.$router.push({path: 'goodsDetails/productId=' + id})
      },
      _addCart (id, name, img) {
        if (!this.showMoveImg) {     // 动画是否在运动
          if (this.login) { // 登录了 直接存在用户名下
            addCart({goods_id: id}).then(res => {
              if (res.code === 1) {
                this.$message({
                  type: 'success',
                  message: '加入购物车成功！'
                })
                return userInfo()
              }
            }).then(res => {
              this.UPDATE_USER(res.data)
            })
          } else { // 未登录 vuex
            this.$router.push('/login')
          }
          // 加入购物车动画
          var dom = event.target
          // 获取点击的坐标
          let elLeft = dom.getBoundingClientRect().left + (dom.offsetWidth / 2)
          let elTop = dom.getBoundingClientRect().top + (dom.offsetHeight / 2)
          // 需要触发
          this.ADD_ANIMATION({moveShow: true, elLeft: elLeft, elTop: elTop, img: img})
          if (!this.showCart) {
            this.SHOW_CART({showCart: true})
          }
        }
      }
    },
    computed: {
      ...mapState(['login', 'showMoveImg', 'showCart'])
    },
    mounted () {
    },
    components: {
      YButton
    }
  }
</script>
<style lang="scss" rel="stylesheet/scss" scoped>
  @import "../assets/style/mixin";
  @import "../assets/style/theme";

  .good-item {
    background: #fff;
    width: 25%;
    transition: all .5s;
    height: 345px;
    &:hover {
      transform: translateY(-3px);
      box-shadow: 1px 1px 20px #999;
      .good-price p {
        display: none;
      }
      .ds {
        display: flex;
      }
    }
    .ds {
      @extend %block-center;
      width: 100%;
      display: none;
    }

    .good-img {
      img {
        margin: 50px auto 10px;
        @include wh(206px);
        display: block;
      }
    }
    .good-price {
      margin: 15px 0;
      height: 30px;
      text-align: center;
      line-height: 30px;
      color: #e4393c;
      font-size: 20px;
    }
    .good-title {
      line-height: 1.2;
      font-size: 16px;
      color: inherit;
      margin: 0 auto;
      padding: 0 14px;
      text-align: center;
      overflow: hidden;
    }
    h3 {
      text-align: center;
      line-height: 1.2;
      font-size: 12px;
      color: #d0d0d0;
      padding: 10px;
    }

  }
</style>
