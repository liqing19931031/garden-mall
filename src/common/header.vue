        ...
        <div class="w-box">
          <div class="nav-logo">
            <h1 @click="changePage(1)">
              <router-link to="/" title="园林链 商城">园林链 商城</router-link>
            </h1>
          </div>
          <div class="right-box">
            <div class="nav-list">
            ...
            <div class="nav-aside" ref="aside" :class="{fixed:st}">
              ...
              <div class="shop pr" @mouseover="cartShowState(true)" @mouseout="cartShowState(false)"
                ...
                <span class="cart-num">
                  <i class="num" :class="{no:!this.$store.state.userInfo.cart_num, move_in_cart:receiveInCart}">{{this.$store.state.userInfo.cart_num || 0}}</i></span>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default{
    ...
    computed: {
      ...
      totalNum () {
        console.log(+this.$store.state.userInfo.cart_num)
        return +this.$store.state.userInfo ? +this.$store.state.userInfo.cart_num : 0
      }
    },
    methods: {
